...
const buildHttpDecorator = (options: IBuildDecoratorOption): MethodDecorator => {
    const { errorMessage, successMessage, errorCode, successCode } = options;
...
export function handle(...args) {
  const option = args[0];
  const isOption = (value: THandleOption): value is IHandleOption => value instanceof Object;
  ...
  return buildHttpDecorator({ errorCode, successCode, errorMessage, successMessage, });
}

export const HttpProcessor = { error, success, handle };
